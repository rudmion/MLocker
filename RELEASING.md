...
3. Извлекает `tag_name` из ответа (например, `v0.7.0`), убирает префикс `v`
4. Сравнивает версию из тега с текущей через `compare_versions()`
5. Если тег новее — возвращает `has_update: true`
...
- Если обновление есть — показывает диалог с changelog и кнопкой «Установить»
- Кнопка «Установить» открывает страницу релиза в браузере
...
### «У меня последняя версия», но на GitHub есть новый коммит

Причина: коммит запушен, но тег нет. updater проверяет **релизы** (создаётся workflow при пуше тега), а не коммиты.

Решение: создайте и запушьте тег.
